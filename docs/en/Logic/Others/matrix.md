...
| | This simple circuit can analyze up to 20 (4 * 5) keys. However, it should be noted here that only in cases a number of keys can be pressed simultaneously.  The controller can handle with this circuit, several buttons in a column in any doubt, but not when keys are pressed simultaneously on different columns. The wiring may be extended by each button is decoupled via diodes, and thus the influence of different 
