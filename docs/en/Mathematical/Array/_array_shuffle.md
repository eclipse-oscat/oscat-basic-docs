...
Example:	_ARRAY_SHUFFLE (ADRs(bigarray), SIZEOF(bigarray)) A call of the function _ARRAY_SHUFFLE could change an array as follows. Since the function uses a  pseudo  random  algorithm each time the result is different, the results are not reproducible, even through a restart of the program or the programmable logic control (plc).
